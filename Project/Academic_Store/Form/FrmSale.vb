...
Public Class FrmSale
    ...
    Dim mQry As String = "", mSearchCode As String = "", mNCatStr$ = ""

    Private Const Col_SNo As Byte = 0
    Public WithEvents DGL1 As New AgControls.AgDataGrid
    Private Const Col1Item As Byte = 1
    Private Const Col1ItemDescription As Byte = 2
    Private Const Col1Batch As Byte = 3
    Private Const Col1ItemNature1 As Byte = 4
    Private Const Col1ItemNature2 As Byte = 5
    Private Const Col1Godown As Byte = 6
    Private Const Col1Qty As Byte = 7
    Private Const Col1Rate As Byte = 8
    Private Const Col1Amount As Byte = 9
    Private Const Col1Addition As Byte = 10
    Private Const Col1Deduction As Byte = 11
    Private Const Col1NetAmount As Byte = 12
    Private Const Col1Addition_H As Byte = 13
    Private Const Col1Deduction_H As Byte = 14
    Private Const Col1LandedAmount As Byte = 15

    'Const by akash on date 18-9-10
    Private FormType As Byte
    Private Const SaleEntry As Byte = 0
    Private Const SaleEntryAuthenticated As Byte = 1
    ...
    Private Sub IniGrid()
        ...
        With AgCL
            ...
            .AddAgTextColumn(DGL1, "DGL1Item", 180, 30, "Item", True, False, False)
            ...
            .AddAgNumberColumn(DGL1, "DGL1Qty", 80, 8, 3, False, "Qty", True, False, True)
            ...
            .AddAgNumberColumn(DGL1, "DGL1Addition", 80, 8, 2, False, "Addition", True, False, True)
            .AddAgNumberColumn(DGL1, "DGL1Deduction", 80, 8, 2, False, "Deduction", True, False, True)
            .AddAgNumberColumn(DGL1, "DGL1NetAmount", 100, 8, 2, False, "Net Amount", True, True, True)
            .AddAgNumberColumn(DGL1, "DGL1Addition_H", 80, 8, 2, False, "Addition_H", False, True, True)
            .AddAgNumberColumn(DGL1, "DGL1Deduction_H", 80, 8, 2, False, "Deduction_H", False, True, True)
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If AgL.StrCmp(Me.Text, "Sale Entry") Then
                FormType = SaleEntry
            ElseIf AgL.StrCmp(Me.Text, "Sale Entry {Authenticated}") Then
                FormType = SaleEntryAuthenticated
            End If
            'end change 

            mNCatStr = "" & AgL.Chk_Text(ClsVar.NCat_StoreSale) & ""
    ...
    Private Sub FIniMaster(Optional ByVal BytDel As Byte = 0, Optional ByVal BytRefresh As Byte = 1)
        ...
        If FormType = SaleEntry Then
            bCondStr = " AND P.ApprovedDate IS  NULL "
        ElseIf FormType = SaleEntryAuthenticated Then
    ...
    Sub Ini_List()
        ...
        'mQry = "Select Code As Code, Description As Name, SaleRate,s.Stock   " & _
        '        " From Store_Item LEFT JOIN (SELECT item,sum(isnull(qty_rec,0))-sum(isnull(qty_iss,0)) AS Stock FROM Store_Stock GROUP BY item) s ON Store_Item.Code=s.item " & _
        '        " Where 1=1 " & _
        '        " Order By Description"
        mQry = "Select Store_Item.code As Code, Store_Item.Description As Name, Store_Item.SaleRate,s.Stock,store_unit.Description as Unit   " & _
    ...
    Private Sub IniBatchHelp(Optional ByVal All_Records As Boolean = True)
        ...
        End If
        DGL1.AgHelpDataSet(Col1Batch, , , , , False) = AgL.FillData(mQry, AgL.GCn)
        ...
        Try
            ...
            If DTMaster.Rows.Count > 0 Then
                If MsgBox("Are You Sure To Delete This Record?", MsgBoxStyle.Question + MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, AgLibrary.ClsMain.PubMsgTitleInfo) = vbYes Then
                    ...
                    AgL.Dman_ExecuteNonQry("Delete From Store_SaleLedgerM Where DocId = '" & mSearchCode & "' ", AgL.GCn, AgL.ECmd)

                    AgL.LedgerUnPost(AgL.GCn, AgL.ECmd, mSearchCode)

                    mQry = "Delete From Store_SaleStockHeader Where DocId = '" & mSearchCode & "' "
                    ...
                    mQry = "Delete From Store_Sale Where DocId = '" & mSearchCode & "'"
                    AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
    ...
    Private Sub Topctrl1_tbSave() Handles Topctrl1.tbSave
        ...
        Try
            ...
            If Topctrl1.Mode = "Add" Then
                mQry = "INSERT INTO Store_Sale " & _
                        " ( DocId, Div_Code, Site_Code, V_Type, V_Prefix, V_No, V_Date, " & _
                        " AcCode, Amount, Addition, Deduction, NetAmount, Addition_H, " & _
                        " Deduction_H, InvoiceAmount, Remark, Department, " & _
                        " PreparedBy, U_EntDt, U_AE ,ReferenceNo) " & _
                        " VALUES ( " & _
                        " '" & mSearchCode & "', " & AgL.Chk_Text(AgL.PubDivCode) & ", " & AgL.Chk_Text(TxtSite_Code.AgSelectedValue) & ", " & _
                        " " & AgL.Chk_Text(TxtV_Type.AgSelectedValue) & ", " & AgL.Chk_Text(LblPrefix.Text) & ", " & Val(TxtV_No.Text) & "," & AgL.ConvertDate(TxtV_Date.Text) & ", " & _
                        " " & AgL.Chk_Text(TxtAcCode.AgSelectedValue) & ", " & Val(TxtAmount.Text) & ", " & Val(TxtAddition.Text) & ", " & Val(TxtDeduction.Text) & ", " & Val(TxtNetAmount.Text) & ", " & _
                        " " & Val(TxtAddition_H.Text) & ", " & Val(TxtDeduction_H.Text) & "," & Val(TxtInvoiceAmount.Text) & ", " & AgL.Chk_Text(TxtRemark.Text) & ", " & AgL.Chk_Text(TxtDepartment.AgSelectedValue) & "," & _
                        " " & AgL.Chk_Text(AgL.PubUserName) & "," & AgL.ConvertDate(AgL.PubLoginDate) & ", 'A'," & AgL.Chk_Text(TxtRefNo.Text) & ")"
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
            ...
            Else
                mQry = "Update Store_Sale " & _
                        ...
                        " 	Addition = " & Val(TxtAddition.Text) & ", " & _
                        " 	Deduction = " & Val(TxtDeduction.Text) & ", " & _
                        " 	NetAmount = " & Val(TxtNetAmount.Text) & ", " & _
                        " 	Addition_H = " & Val(TxtAddition_H.Text) & ", " & _
                        " 	Deduction_H = " & Val(TxtDeduction_H.Text) & ", " & _
                        " 	InvoiceAmount = " & Val(TxtInvoiceAmount.Text) & ", " & _
            ...
            If Topctrl1.Mode = "Edit" Then
                AgL.Dman_ExecuteNonQry("Delete From Store_SaleStockHeader Where DocId = '" & mSearchCode & "' ", AgL.GCn, AgL.ECmd)
                ...
                For I = 0 To .Rows.Count - 1
                    If .Item(Col1Item, I).Value <> "" Then
                        ...
                        mQry = "INSERT INTO Store_Stock " & _
                                ...
                                " Item_Nature2,BatchNo,Qty_Iss,Rate,Amount, " & _
                                " Addition,Deduction,NetAmount,Addition_H,Deduction_H, " & _
                                " LandedAmount, Godown " & _
                                " ) " & _
                                " VALUES " & _
                                " ( " & _
                                ...
                                " " & Val(.Item(Col1LandedAmount, I).Value) & ", " & AgL.Chk_Text(.AgSelectedValue(Col1Godown, I)) & " " & _
                                " )"
                        AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
                    End If
                Next I
            ...
            mQry = "INSERT INTO Store_SaleStockHeader (DocId) Values ('" & mSearchCode & "') "
            AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)

            'changed by akash on date 17-9-10

            'Call AccountPosting()
            'mQry = "INSERT INTO Store_SaleLedgerM (DocId) Values ('" & mSearchCode & "') "
            'AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
    ...
    Public Sub MoveRec()
        ...
        Try
            ...
            If DTMaster.Rows.Count > 0 Then
                ...
                mQry = "Select P.*, S.Add1, S.Add2, S.Add3, C.CityName, V.NCat " & _
                      "From Store_Sale P " & _
                      "Left Join Subgroup S On P.AcCode = S.SubCode " & _
                      "Left Join City C On S.CityCode = C.CityCode " & _
                      "Left Join Voucher_Type v On V.V_Type = P.V_Type " & _
    ...
    Private Sub DispText(Optional ByVal Enb As Boolean = False)
        ...
        LblAddition.Text = ClsVar.SaleAddition_Text
        LblDeduction.Text = ClsVar.SaleDeduction_Text
        LblAddition_H.Text = ClsVar.SaleAddition_H_Text
        LblDeduction_H.Text = ClsVar.SaleDeduction_H_Text

        TxtSite_Code.Enabled = False : TxtV_No.Enabled = False
        TxtAddress1.Enabled = False : TxtAddress2.Enabled = False : TxtAddress3.Enabled = False : TxtCity.Enabled = False
        TxtNetAmount.Enabled = False : TxtAmount.Enabled = False : TxtAddition.Enabled = False : TxtDeduction.Enabled = False

        'Code by Akash on date 18-9-10
        If Topctrl1.Mode = "Browse" Then GroupBox1.Visible = True
        If FormType = SaleEntryAuthenticated Then Topctrl1.tAdd = False : Topctrl1.tEdit = False
        'End Change                                     
        ...
        'Code by Akash on date 18-9-10
        'If FormType = SaleEntryAuthenticated Then
        '    GroupBox1.Visible = False : Topctrl1.tAdd = False : Topctrl1.tEdit = False
        'ElseIf FormType = SaleEntry Then
                    ...
                    Else
                        If Me.Controls("HelpDg") IsNot Nothing And CType(Me.Controls("HelpDg"), AgControls.AgDataGrid).CurrentCell IsNot Nothing Then
                            With CType(Me.Controls("HelpDg"), AgControls.AgDataGrid)
                                DGL1.Item(Col1Rate, mRowIndex).Value = AgL.VNull(.Item("SaleRate", .CurrentCell.RowIndex).Value)
    ...
    Private Sub Control_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles _
        TxtSite_Code.Validating, TxtV_Date.Validating, TxtV_Type.Validating, _
        TxtAcCode.Validating, TxtAmount.Validating, TxtAddition.Validating, TxtAddition_H.Validating, TxtDeduction.Validating, _
        TxtDeduction_H.Validating, TxtInvoiceAmount.Validating, TxtNetAmount.Validating, TxtRefNo.Validating
    ...
    Private Function AccountPosting() As Boolean
        ...
        LedgAry(I).ContraSub = ClsVar.SaleAc
        LedgAry(I).AmtDr = Val(TxtInvoiceAmount.Text)
        LedgAry(I).AmtCr = 0
        LedgAry(I).Narration = ""
        ...
        If Val(TxtAmount.Text) > 0 Then
            ...
            LedgAry(I).SubCode = ClsVar.SaleAc
        ...
        If Val(TxtAddition.Text) > 0 Then
            ...
            LedgAry(I).SubCode = ClsVar.SaleAdditionAc
            LedgAry(I).ContraSub = TxtAcCode.AgSelectedValue
            LedgAry(I).AmtDr = 0
            LedgAry(I).AmtCr = Val(TxtAddition.Text)
            LedgAry(I).Narration = ""
        End If
        ...
        If Val(TxtDeduction.Text) > 0 Then
            ...
            LedgAry(I).SubCode = ClsVar.SaleDeductionAc
            LedgAry(I).ContraSub = ClsVar.SaleAc
            LedgAry(I).AmtDr = Val(TxtDeduction.Text)
            LedgAry(I).AmtCr = 0
            LedgAry(I).Narration = ""
        End If
        ...
        If Val(TxtAddition_H.Text) > 0 Then
            ...
            LedgAry(I).SubCode = ClsVar.SaleAddition_HAc
            LedgAry(I).ContraSub = TxtAcCode.AgSelectedValue
            LedgAry(I).AmtDr = 0
            LedgAry(I).AmtCr = Val(TxtAddition_H.Text)
            LedgAry(I).Narration = ""
        End If
        ...
        If Val(TxtDeduction_H.Text) > 0 Then
            ...
            LedgAry(I).SubCode = ClsVar.SaleDeduction_HAc
            LedgAry(I).ContraSub = ClsVar.SaleAc
            LedgAry(I).AmtDr = Val(TxtDeduction_H.Text)
            LedgAry(I).AmtCr = 0
            LedgAry(I).Narration = ""
        End If
        ...
        If TxtRemark.Text.Trim = "" Then
            mCommNarr = "Being Goods Sold To """ & TxtAcCode.Text & """ Agt. Voucher Date " & TxtV_Date.Text & " Voucher No " & AgL.ConvertDocId(mSearchCode)
        Else
        ...
        Try
            If Topctrl1.Mode = "Browse" Then
                ...
                If DTMaster.Rows.Count > 0 Then
                    ...
                    AgL.ECmd.CommandText = "Update Store_Sale Set ApprovedBy='" & AgL.PubUserName & "', " & _
