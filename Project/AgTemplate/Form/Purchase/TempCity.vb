...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If TxtCityName.Text.Trim = "" Then Err.Raise(1, , "City Name Is Required!")
        ...
        If Topctrl1.Mode = "Add" Then
            ...
            mQry = "Select count(*) From City_Log Where CityName='" & TxtCityName.Text & "' And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "City Name Already Exists in Log File")
        Else
            mQry = "Select count(*) From City Where CityName ='" & TxtCityName.Text & "' And CityCode<>'" & mInternalCode & "'  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "City Name Already Exist!")

            mQry = "Select count(*) From City_Log Where CityName='" & TxtCityName.Text & "' And UID <>'" & mSearchCode & "' And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "City Name Already Exists in Log File")
        End If
    End Sub

    Private Sub FrmYarn_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        AgL.PubFindQry = "SELECT UID, CityName , State, Country " & _
                        " FROM City_Log " & _
    ...
    Private Sub FrmYarn_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        AgL.PubFindQry = "SELECT CityCode, CityName , State, Country " & _
                            " FROM City " & _
    ...
    Private Sub FrmYarn_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        mQry = "Update City_Log " & _
                "   SET  " & _
                "	CityName = " & AgL.Chk_Text(TxtCityName.Text) & ", " & _
    ...
    Private Sub FrmQuality1_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        ...
        mQry = "Select CityCode as Code, CityName From City " & _
            "  Order By CityName "
        TxtCityName.AgHelpDataSet() = AgL.FillData(mQry, AgL.GCn)

        mQry = "Select Distinct State as Code, State, Country  " & _
                " From City  " & _
                " WHERE State Is Not Null " & _
                " Order By State "
        TxtState.AgHelpDataSet(1) = AgL.FillData(mQry, AgL.GCn)

        mQry = "Select Distinct Country as Code, Country " & _
                " From City  " & _
                " WHERE Country Is Not Null " & _
                " Order By Country "
    ...
    Private Sub FrmYarn_BaseFunction_FIniMast(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMast
        mQry = "Select CityCode As SearchCode " & _
                " From City " & _
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        If FrmType = ClsMain.EntryPointType.Main Then
            mQry = "Select * " & _
                " From City Where CityCode='" & SearchCode & "'"
        Else
            mQry = "Select * " & _
                " From City_Log Where UID='" & SearchCode & "'"
        End If
