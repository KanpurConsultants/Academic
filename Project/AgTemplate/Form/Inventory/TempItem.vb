...
#Region "Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.TxtItemGroup.AgMasterHelp = True
...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From Item Where Description='" & TxtDescription.Text & "' And " & ClsMain.RetDivFilterStr & " And IsNull(IsDeleted,0) = 0  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Item Description Already Exist!")

            mQry = "Select count(*) From Item_Log Where Description='" & TxtDescription.Text & "' And " & ClsMain.RetDivFilterStr & "  And IsNull(IsDeleted,0) = 0  And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''   "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Item Description Already Exists in Log File")
        Else
            mQry = "Select count(*) From Item Where Description='" & TxtDescription.Text & "' And Code<>'" & mInternalCode & "' And " & ClsMain.RetDivFilterStr & "  And IsNull(IsDeleted,0) = 0  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Item Description Already Exist!")

            mQry = "Select count(*) From Item_Log Where Description = '" & TxtDescription.Text & "' And UID <>'" & mSearchCode & "' And " & ClsMain.RetDivFilterStr & "  And IsNull(IsDeleted,0) = 0  And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Item Description Already Exists in Log File")
        End If
    ...
    Private Sub FrmYarn_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        mQry = "UPDATE Item_Log " & _
                ...
                " ItemGroup = " & AgL.Chk_Text(TxtItemGroup.Text) & ", " & _
                " UPCCode = " & AgL.Chk_Text(TxtUPCCode.Text) & ", " & _
                " SalesTaxPostingGroup = " & AgL.Chk_Text(TxtSalesTaxPostingGroup.Text) & ", " & _
                " ExcisePostingGroup = " & AgL.Chk_Text(TxtExciseGroup.Text) & ", " & _
                " EntryTaxPostingGroup = " & AgL.Chk_Text(TxtEntryTaxGroup.Text) & " " & _
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Buyer, I).Value <> "" Then
                    mQry = "INSERT INTO ItemBuyer_Log (UID, Code, Sr, Buyer, BuyerSku, BuyerUpcCode) " & _
                           "VALUES (" & AgL.Chk_Text(SearchCode) & ", " & AgL.Chk_Text(mInternalCode) & ", " & _
                           " " & Val(I) & ", " & AgL.Chk_Text(.AgSelectedValue(Col1Buyer, I)) & ", " & _
                           " " & AgL.Chk_Text(.Item(Col1BuyerSku, I).Value) & ",  " & _
    ...
    Private Sub FrmQuality1_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        mQry = "Select Distinct ItemGroup as Code, ItemGroup as [Item Group], Div_Code From Item Order By ItemGroup"
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                TxtItemGroup.Text = AgL.XNull(.Rows(0)("ItemGroup"))
    ...
    Private Sub TxtItemGroup_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtItemGroup.Enter
        Select Case sender.name
            Case TxtItemGroup.Name
                'sender.AgRowFilter = " Div_Code = '" & TxtDivision.AgSelectedValue & "' "
        End Select
    End Sub
End Class
