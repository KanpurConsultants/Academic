...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.V_Type AS [Exchange Type], H.V_Prefix AS Prefix, H.V_Date AS Date, H.V_No AS [Exchange No], " & _
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Exchange Type], H.V_Prefix AS Prefix, H.V_Date AS Date, H.V_No AS [Exchange No], " & _
                        " H.ManualRefNo AS [Manual No], H.Process, H.DueDate AS [Due Date], H.IssQty AS [Issue Qty], H.IssMeasure AS [Issue Measure],  " & _
                        " H.RecQty AS [Rec Qty], H.RecMeasure AS [Rec Measure], H.JobReceiveFor AS [Job Receive For], H.Remarks, H.Structure, H.EntryBy AS [Entry By],  " & _
                        " H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status], H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date],  " & _
                        " H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status, H.BillingType AS [Billing Type], H.OrderBy AS [ORDER By],  " & _
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE JobIssRec_Log " & _
                ...
                " RecQty = " & Val(LblTotalReceivedQty.Text) & ", " & _
                " RecMeasure = " & Val(LblTotalReceivedMeasure.Text) & ", " & _
        ...
        With Dgl1
            For I = 0 To Dgl1.RowCount - 1
                If Dgl1.Item(Col1ReceiveItem, I).Value <> "" Then
                    mSr += 1
                    mQry = " INSERT INTO JobExchangeDetail_Log (UID, DocId, Sr, ReceiveItem, " & _
                            " ReceiveQty, ReceiveItemUnit, " & _
                            " ReceiveItemMeasurePerPcs, ReceiveItemTotalMeasure, " & _
                            " ReceiveItemMeasureUnit, IssueItem, IssueQty, IssueItemUnit, " & _
                            " IssueItemMeasurePerPcs, IssueItemTotalMeasure, IssueItemMeasureUnit, " & _
                            " JobOrder) " & _
                            " Values (" & AgL.Chk_Text(mSearchCode) & ", " & AgL.Chk_Text(mInternalCode) & "," & _
                            " " & mSr & ", " & AgL.Chk_Text(.AgSelectedValue(Col1ReceiveItem, I)) & ", " & _
                            " " & Val(.Item(Col1ReceiveQty, I).Value) & ", " & _
                            " " & AgL.Chk_Text(.Item(Col1ReceiveItemUnit, I).Value) & ", " & _
                            " " & Val(.Item(Col1ReceiveItemMeasurePerPcs, I).Value) & ", " & _
                            " " & Val(.Item(Col1ReceiveItemTotalMeasure, I).Value) & ", " & _
                            " " & AgL.Chk_Text(.Item(Col1ReceiveItemMeasureUnit, I).Value) & ", " & _
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1IssueItem, I)) & ", " & _
                            " " & Val(.Item(Col1IssueQty, I).Value) & ", " & _
                            " " & AgL.Chk_Text(.Item(Col1IssueItemUnit, I).Value) & ", " & _
                            " " & Val(.Item(Col1IssueItemMeasurePerPcs, I).Value) & ", " & _
                            " " & Val(.Item(Col1IssueItemTotalMeasure, I).Value) & ", " & _
                            " " & AgL.Chk_Text(.Item(Col1IssueItemMeasureUnit, I).Value) & ", " & _
    ...
    Private Sub FrmProductionOrder_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        ...
        TxtJobOrderNo.AgHelpDataSet(6, TabControl1.Top + TP1.Top, TabControl1.Left + TP1.Left) = HelpDataSet.JobOrder
            ...
            For I = 0 To .Rows.Count - 1
                If .Item(Col1ReceiveItem, I).Value <> "" Then
                    ...
                    If AgTemplate.ClsMain.FunRetStock(.AgSelectedValue(Col1IssueItem, I), mInternalCode, , TxtGodown.AgSelectedValue, , AgTemplate.ClsMain.StockStatus.Standard, TxtV_Date.Text) < Val(.Item(Col1IssueQty, I).Value) Then
                        MsgBox("Qty of " & .Item(Col1IssueItem, I).Value & " In " & TxtGodown.Text & " is less than " & Dgl1.Item(Col1IssueQty, I).Value & "", MsgBoxStyle.Information)
                        .CurrentCell = .Item(Col1IssueQty, I) : Dgl1.Focus()
                        passed = False : Exit Sub
                    End If
                End If
    ...
    Private Sub Txt_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtGodown.Enter
        Select Case sender.name
            Case TxtGodown.Name
                TxtGodown.AgRowFilter = " Site_Code = '" & TxtSite_Code.AgSelectedValue & "' " & _
                    " And IsDeleted = 0 " & _
    ...
    Private Sub TempJobIssue_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        mQry = " SELECT I.Code, I.Description AS Item, I.ItemType, I.SalesTaxPostingGroup, I.Unit, " & _
                " I.Measure, I.MeasureUnit, IsNull(I.IsDeleted,0) AS IsDeleted, " & _
        ...
        Try
            ...
            Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
                Case Col1ReceiveItem, Col1IssueItem
                    Dgl1.AgRowFilter(Dgl1.Columns(Col1ReceiveItem).Index) = " IsDeleted = 0 " & _
            ...
            Select Case sender.Name
                Case TxtJobOrderNo.Name
                    TxtJobOrderNo.AgRowFilter = " IsDeleted = 0 " & _
                        ...
                        " And JobOrderDate <= '" & TxtV_Date.Text & "' " & _
