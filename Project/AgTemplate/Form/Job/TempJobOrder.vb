...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocId AS SearchCode, H.V_Type AS [ORDER Type], H.V_Date AS [ORDER Date],  " & _
                    ...
                    " LEFT JOIN JobOrder  JO ON H.JobOrderDocID  =JO.DocID  " & _
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE JobOrder_Log " & _
                ...
                " JobOrderDocID = " & AgL.Chk_Text(TxtJobOrderNo.AgSelectedValue) & " " & _
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    ...
                    mQry = "INSERT INTO JobOrderDetail_Log(UID, DocId, Sr, " & _
                            ...
                            " ProdPlan, ProdOrder,LossPer, Loss, Rate, Amount, ReceiveQty, ReceiveMeasure, CancelQty, CancelMeasure, ReceiveLoss, Bom) " & _
                            ...
                            " " & Val(.Item(Col1ReceiveMeasure, I).Value) & ", " & Val(.Item(Col1CancelQty, I).Value) & ", " & Val(.Item(Col1CancelMeasure, I).Value) & ", " & Val(.Item(Col1ReceiveLoss, I).Value) & "," & AgL.Chk_Text(.AgSelectedValue(Col1BOM, I)) & ") "
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                TxtJobOrderNo.AgSelectedValue = AgL.XNull(.Rows(0)("JobOrderDocId"))
                ...
                With DsTemp.Tables(0)
                    ...
                    If .Rows.Count > 0 Then
                        For I = 0 To DsTemp.Tables(0).Rows.Count - 1
                            ...
                            Dgl1.Item(Col1ReceiveQty, I).Value = AgL.VNull(.Rows(I)("ReceiveLoss"))
    ...
    Private Sub FrmProductionOrder_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        ...
        TxtOrderBy.AgHelpDataSet(0, TabControl1.Top + TP1.Top, TabControl1.Left + TP1.Left) = HelpDataSet.OrderBy
        ...
        TxtJobOrderNo.AgHelpDataSet(4, TabControl1.Top + TP1.Top, TabControl1.Left + TP1.Left) = HelpDataSet.JobOrder
        ...
        Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
            Case Col1Item
                If mJobOrderType = ClsMain.JobOrderType.JobOrder Or mJobOrderType = ClsMain.JobOrderType.JobOrder_With_Issue Then
                    If AgL.StrCmp(TxtJobOrderFor.Text, "Stock") Then
                        Call IniItemList()
    ...
    Private Sub Txt_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtManualRefNo.Enter, TxtJobOrderNo.Enter, TxtJobWorker.Enter
        Try
            Select Case sender.name
                Case TxtGodown.Name
    ...
    Private Sub TempJobOrder_BaseEvent_Approve_PreTrans(ByVal SearchCode As String) Handles Me.BaseEvent_Approve_PreTrans
        If mJobOrderType = ClsMain.JobOrderType.JobOrder Then
            mQry = " SELECT H.DocID, L.Item, L.Qty, L.TotalMeasure, L.ProdOrder " & _
                    ...
                    " LEFT JOIN JobOrderDetail L With (Nolock) ON H.DocID=L.DocId  " & _
                    " WHERE H.DocID = " & AgL.Chk_Text(mInternalCode) & " "
            DsMain = AgL.FillData(mQry, AgL.GcnRead)
        ElseIf mJobOrderType = ClsMain.JobOrderType.JobOrder_Cancel Then
            mQry = " SELECT H.DocID, H.JobOrderDocId As JobOrder, L.Item,L.Qty, L.TotalMeasure ,L.ProdOrder " & _
                    " FROM JobOrder H With (Nolock) " & _
                    " LEFT JOIN JobOrderDetail L With (Nolock) ON H.DocID = L.DocId  " & _
                    " WHERE H.DocID = " & AgL.Chk_Text(mInternalCode) & " "
            DsMain = AgL.FillData(mQry, AgL.GcnRead)
    ...
    Private Sub ProcUpDateProdOrder(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand)
        Dim I As Integer = 0
            ...
            If .Rows.Count > 0 Then
                For I = 0 To DsMain.Tables(0).Rows.Count - 1
                    mQry = " UPDATE MaterialPlanDetail " & _
                            " SET JobOrderQty = (SELECT IsNull(Sum(L.Qty),0)-IsNull(Sum(L.CancelQty),0)  " & _
                            " 	                 FROM JobOrderDetail L With (NoLock)  " & _
                            "                    Left Join JobOrder H  With (NoLock) On L.DocID = H.DocID " & _
                            "                    Left Join Voucher_Type V  With (NoLock) On H.V_Type = V.V_Type " & _
                            " 	                 WHERE L.ProdOrder = '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' " & _
                            " 	                 AND Item = '" & AgL.XNull(.Rows(I)("Item")) & "' " & _
                            "                    And V.NCat Not In (Select ProcessCancelNCat from Process  With (NoLock)  Where ProcessCancelNCat is not Null)), " & _
                            " JobOrderMeasure = (SELECT IsNull(Sum(L.TotalMeasure),0)-IsNull(Sum(L.CancelMeasure),0) " & _
                            " 	                 FROM JobOrderDetail L   With (NoLock) " & _
                            "                    Left Join JobOrder H  With (NoLock) On L.DocID = H.DocID " & _
                            "                    Left Join Voucher_Type V  With (NoLock) On H.V_Type = V.V_Type " & _
                            " 	                 WHERE L.ProdOrder =  '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' " & _
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mQry = " UPDATE MaterialPlanDetail " & _
                            ...
                            " 	                 AND Item = '" & .AgSelectedValue(Col1Item, I) & "' And V.NCat Not In (Select ProcessCancelNCat from Process  With (NoLock)  Where ProcessCancelNCat is not Null)) , " & _
                            " JobOrderMeasure = (SELECT IsNull(Sum(L.TotalMeasure),0)-IsNull(Sum(L.CancelMeasure),0) " & _
                            " 	                 FROM JobOrderDetail L  " & _
                            "                    Left Join JobOrder H  With (NoLock) On L.DocID = H.DocID " & _
                            "                    Left Join Voucher_Type V  With (NoLock) On H.V_Type = V.V_Type " & _
    ...
    Private Sub ProcFillJobValues()
        ...
        Try
            mQry = " SELECT L.Parameter, L.StdValue  " & _
                    ...
                    " Where L.Code = (SELECT P.QcGroup FROM Process P WHERE P.NCat = '" & mJobProcess & "') "
            DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
            With DtTemp
                Dgl3.RowCount = 1
                Dgl3.Rows.Clear()
    ...
    Private Sub TempJobOrder_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        mQry = "SELECT I.Code, I.Description, I.Unit, I.ItemType, I.SalesTaxPostingGroup , " & _
                " IsNull(I.IsDeleted ,0) AS IsDeleted, I.Div_Code, I.Status, " & _
        ...
        mQry = "SELECT H.Code, H.Description,  " & _
                " IsNull(H.IsDeleted ,0) AS IsDeleted, H.Div_Code, H.Status " & _
                " FROM BOM H"
        ...
        mQry = " SELECT I.Code, I.Description AS Item, Po.V_Date AS [Prod.Order Date], PO.ManualRefNo as [Prod.Order No.], " & _
        " IsNull(L.UserMaterialPlanQty,0) - (IsNull(L.ProdOrdQty,0) + IsNull(L.JobOrderQty,0) + IsNull(L.PurchOrdQty,0)) AS PendingQty, " & _
        " I.Unit, I.ItemType, I.ItemGroup, I.ItemCategory, I.SalesTaxPostingGroup,     " & _
        " IsNull(I.IsDeleted ,0) AS IsDeleted, H.ProdOrder,  I.Div_Code, " & _
        " IsNull(I.Status, '" & ClsMain.EntryStatus.Active & "' ) As Status,  " & _
        " L.MeasurePerPcs, I.MeasureUnit, " & _
        " IsNull(L.UserMaterialPlanMeasure,0) - (IsNull(L.ProdOrdMeasure,0)  + IsNull(L.JobOrderMeasure,0)  + IsNull(L.PurchOrdMeasure,0)) as PendingMeasure" & _
        " FROM (Select * from MaterialPlan Where IsNull(IsDeleted,0)=0) H  " & _
        " LEFT JOIN MaterialPlanDetail L ON L.DocId = H.DocID " & _
        " LEFT JOIN (Select * from Item Where IsNull(IsDeleted,0)=0) I ON L.Item = I.Code " & _
        " LEFT JOIN (Select * from ProdOrder Where IsNull(IsDeleted,0)=0) Po ON H.ProdOrder = Po.DocID " & _
        " WHERE H.ProdOrder Is Not NULL "
        HelpDataSet.ItemFromProdOrder = AgL.FillData(mQry, AgL.GCn)


        mQry = " SELECT I.Code, I.Description As Item, L.ProdOrder, IsNull(L.Qty,0) - IsNull(L.CancelQty,0) As PendingQty, " & _
                " L.Unit, L.MeasurePerPcs, I.ItemType, I.ItemGroup, I.ItemCategory, " & _
                " IsNull(L.TotalMeasure,0) - IsNull(L.CancelMeasure,0) As PendingMeasure , L.MeasureUnit, L.ProdPlan, " & _
        ...
        mQry = "SELECT J.DocId, J.ManualRefNo As JobOrderNo, " & _
                " IsNull(J.IsDeleted,0) As IsDeleted, J.Div_Code,Vt.NCat, " & _
                " IsNull(J.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status " & _
                " FROM JobOrder J  With (NoLock) " & _
                " LEFT JOIN Voucher_Type Vt  With (NoLock) ON J.V_Type = Vt.V_Type "
        HelpDataSet.JobOrder = AgL.FillData(mQry, AgL.GcnRead)

        mQry = "Select * from JobWorkerRateDetail With (NoLock) Where Process In ('" & EntryNCat & "')"
        HelpDataSet.JobRate = AgL.FillData(mQry, AgL.GcnRead)

        mQry = " SELECT H.Code AS Code, H.Description, H.Process, IsNull(H.IsDeleted,0) as IsDeleted, IsNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status FROM JobWorkerRateGroup H  "
    ...
    Private Sub ProcFillOrderForCancel(ByVal bOrderDocId As String)
        ...
        mQry = " SELECT H.DocID, H.V_Type, H.V_Prefix, H.V_Date, H.V_No, H.Div_Code, H.Site_Code, H.ManualRefNo, " & _
                ...
                " H.JobWithMaterialYN, H.BillingType, H.PurjaNo, H.JobOrderDocID, H.Godown " & _
                " FROM JobOrder H " & _
        ...
        Else
            With DtTemp
                If .Rows.Count > 0 Then
                    ...
                    Dgl1.Item(Col1MeasurePerPcs, RowIndex).Value = Format(AgL.VNull(.Rows(0)("MeasurePerPcs")), "0.000")
    ...
    Private Sub ProcUpDateProdOrderForCancel(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand)
        Dim I As Integer = 0
        With DsMain.Tables(0)
            If .Rows.Count > 0 Then
                For I = 0 To DsMain.Tables(0).Rows.Count - 1
                    mQry = " UPDATE JobOrderDetail " & _
                            " SET CancelQty = (SELECT IsNull(Sum(L.Qty),0)  " & _
                            " 	               FROM JobOrderDetail L  " & _
                            "                  LEFT JOIN JobOrder H On L.DocId = H.DocId " & _
                            " 	               WHERE H.JobOrderDocId = '" & AgL.XNull(.Rows(I)("JobOrder")) & "' " & _
                            " 	               AND Item = '" & AgL.XNull(.Rows(I)("Item")) & "' " & _
                            "                  And ProdOrder = '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' " & _
                            "                  And IsNull(H.IsDeleted,0) = 0), " & _
                            " CancelMeasure = (SELECT IsNull(Sum(L.TotalMeasure),0) " & _
                            " 	               FROM JobOrderDetail L  " & _
                            "                  LEFT JOIN JobOrder H On L.DocId = H.DocId " & _
                            " 	               WHERE H.JobOrderDocId =  '" & AgL.XNull(.Rows(I)("JobOrder")) & "' " & _
                            " 	               AND Item = '" & AgL.XNull(.Rows(I)("Item")) & "' " & _
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mQry = " UPDATE JobOrderDetail " & _
                             ...
                             " 	               FROM JobOrderDetail L  " & _
                             "                 LEFT JOIN JobOrder H On L.DocId = H.DocId " & _
                             " 	               WHERE H.JobOrderDocId = '" & TxtJobOrderNo.AgSelectedValue & "' " & _
