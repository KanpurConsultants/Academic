...
#End Region
    ...
    Private Sub FrmJobWorkerRateMaster_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If TxtItem.AgSelectedValue = "" And TxtItemGroup.AgSelectedValue = "" And TxtItemCategory.AgSelectedValue = "" And TxtJobWorkerRateGroup.Text = "" Then
            mQry = "Select count(*) From JobWorkerRate " & _
                    " Where Process ='" & TxtProcess.AgSelectedValue & "' " & _
                    " And JobWorkerRateGroup Is Null " & _
                    " And Item Is Null " & _
                    " And ItemGroup Is Null " & _
                    " And ItemCategory Is Null " & _
        ...
        If TxtItem.AgSelectedValue = "" And TxtItemGroup.AgSelectedValue = "" And TxtItemCategory.AgSelectedValue = "" And TxtJobWorkerRateGroup.Text <> "" Then
            mQry = "Select count(*) From JobWorkerRate " & _
                    " Where Process ='" & TxtProcess.AgSelectedValue & "' " & _
                    " And JobWorkerRateGroup ='" & TxtJobWorkerRateGroup.AgSelectedValue & "'  " & _
                    " And Item Is Null " & _
                    " And ItemGroup Is Null " & _
                    " And ItemCategory Is Null " & _
                    " And Code <> '" & mInternalCode & "'  "
        ...
        If TxtItem.AgSelectedValue <> "" And TxtJobWorkerRateGroup.Text = "" Then
            mQry = "Select count(*) From JobWorkerRate " & _
                    " Where Process ='" & TxtProcess.AgSelectedValue & "' " & _
                    " And Item = '" & TxtItem.AgSelectedValue & "' " & _
                    " And JobWorkerRateGroup Is Null " & _
                    " And Code <> '" & mInternalCode & "'  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then
                If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then
        ...
        If TxtItemGroup.AgSelectedValue <> "" And TxtItem.AgSelectedValue = "" And TxtJobWorkerRateGroup.Text = "" Then
            mQry = "Select count(*) From JobWorkerRate " & _
                    " Where Process ='" & TxtProcess.AgSelectedValue & "' " & _
                    " And ItemGroup = '" & TxtItemGroup.AgSelectedValue & "' " & _
                    " And Item Is Null " & _
                    " And JobWorkerRateGroup Is Null " & _
                    " And Code <> '" & mInternalCode & "'  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then
        ...
        If TxtItemCategory.AgSelectedValue <> "" And TxtItemGroup.AgSelectedValue = "" And TxtItem.AgSelectedValue = "" And TxtJobWorkerRateGroup.Text = "" Then
            mQry = "Select count(*) From JobWorkerRate " & _
                    " Where Process ='" & TxtProcess.AgSelectedValue & "' " & _
                    " And ItemCategory = '" & TxtItemCategory.AgSelectedValue & "' " & _
                    " And ItemCategory Is Not Null " & _
                    " And ItemGroup Is Null " & _
                    " And Item Is  Null " & _
                    " And JobWorkerRateGroup Is  Null " & _
                    " And Code <> '" & mInternalCode & "'  "
    ...
    Private Sub FrmJobWorkerRateMaster_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1 " & AgL.RetDivisionCondition(AgL, "H.Div_Code") & " AND Isnull(H.IsDeleted,0) = 0 " 
        AgL.PubFindQry = "SELECT H.UID, Vt.Description As Process, I.Description As Item, " & _
                        " Ig.Description As ItemGroup, Ic.Description As ItemCategory " & _
                        " FROM JobWorkerRate_Log H " & _
                        " LEFT JOIN Voucher_Type Vt ON Vt.V_Type = H.Process " & _
                        " LEFT JOIN Item I On H.Item  = I.Code " & _
                        " LEFT JOIN ItemGroup Ig On H.ItemGroup  = Ig.Code " & _
                        " LEFT JOIN ItemCategory Ic On H.ItemCategory = I.Code " & mConStr & _
    ...
    Private Sub FrmJobWorkerRateMaster_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1 " & AgL.RetDivisionCondition(AgL, "H.Div_Code") & " AND Isnull(H.IsDeleted,0) = 0 " 
        AgL.PubFindQry = "SELECT H.Code, Vt.Description As Process,  I.Description As Item, " & _
                        " Ig.Description As ItemGroup, Ic.Description As ItemCategory " & _
                        " FROM JobWorkerRate H " & _
                        " LEFT JOIN Voucher_Type Vt ON Vt.V_Type = H.Process " & _
                        " LEFT JOIN Item I On H.Item  = I.Code " & _
                        " LEFT JOIN ItemGroup Ig On H.ItemGroup  = Ig.Code " & _
                        " LEFT JOIN ItemCategory Ic On H.ItemCategory = I.Code " & mConStr & _
    ...
    Private Sub FrmJobWorkerRateMaster_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        Dim I As Integer = 0, mSr As Integer = 0
        mQry = " Update JobWorkerRate_LOG " & _
                " SET  " & _
                " Process = " & AgL.Chk_Text(TxtProcess.AgSelectedValue) & ", " & _
                " JobWorkerRateGroup = " & AgL.Chk_Text(TxtJobWorkerRateGroup.AgSelectedValue) & ", " & _
                ...
                " GeneralRate = " & Val(TxtGeneralRate.Text) & ", " & _
                " GeneralRateOutside = " & Val(TxtGeneralRateWithoutMaterial.Text) & " " & _
        ...
        mSr += 1
        mQry = " INSERT INTO JobWorkerRateDetail_Log (Code,Sr, " & _
                " Process, " & _
                " JobWorkerRateGroup, " & _
                " Item, " & _
                " ItemGroup, " & _
                " ItemCategory, " & _
                " Rate, " & _
                " RateOutside, " & _
                " UID) " & _
                " VALUES (" & AgL.Chk_Text(mInternalCode) & ", " & Val(mSr) & ", " & _
                " " & AgL.Chk_Text(TxtProcess.AgSelectedValue) & ", " & _
                " " & AgL.Chk_Text(TxtJobWorkerRateGroup.AgSelectedValue) & ", " & _
                " " & AgL.Chk_Text(TxtItem.AgSelectedValue) & ", " & _
                " " & AgL.Chk_Text(TxtItemGroup.AgSelectedValue) & ", " & _
                " " & AgL.Chk_Text(TxtItemCategory.AgSelectedValue) & ", " & _
                " " & Val(TxtGeneralRate.Text) & ", " & _
                " " & Val(TxtGeneralRateWithoutMaterial.Text) & ", " & _
                " '" & mSearchCode & "')"
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1JobWorker, I).Value <> "" Then
                    ...
                    mQry = " INSERT INTO JobWorkerRateDetail_Log (Code,Sr, " & _
                            ...
                            " UID) " & _
                            " VALUES (" & AgL.Chk_Text(mInternalCode) & ", " & Val(mSr) & ", " & _
                            " " & AgL.Chk_Text(.Item(Col1WEF, I).Value) & ", " & _
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1JobWorker, I)) & ", " & _
                            " " & AgL.Chk_Text(TxtProcess.AgSelectedValue) & ", " & _
                            " " & AgL.Chk_Text(TxtJobWorkerRateGroup.AgSelectedValue) & ", " & _
                            " " & AgL.Chk_Text(TxtItem.AgSelectedValue) & ", " & _
                            " " & AgL.Chk_Text(TxtItemGroup.AgSelectedValue) & ", " & _
                            ...
                            " " & Val(.Item(Col1RateOutSide, I).Value) & ", " & _
                            " '" & mSearchCode & "')"
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                End If
            Next
    ...
    Private Sub FrmJobWorkerRateMaster_BaseFunction_FIniMast(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMast
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1 " & AgL.RetDivisionCondition(AgL, "Div_Code") & " AND Isnull(IsDeleted,0) = 0  " 
        mQry = "Select Code As SearchCode " & _
            " From JobWorkerRate " & mConStr & _
    ...
    Private Sub FrmJobWorkerRateMaster_BaseFunction_FIniMastLog(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMastLog
        ...
        mQry = "Select UID As SearchCode " & _
               " From JobWorkerRate_log " & mConStr & _
    ...
    Private Sub FrmJobWorkerRateMaster_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        mQry = " SELECT H.SubCode, Sg.DispName AS JobWorker, IsNull(Sg.IsDeleted,0) AS IsDeleted, " & _
        ...
        mQry = " SELECT H.NCat, L.NCatDescription AS Process " & _
                " FROM Process H " & _
                " LEFT JOIN VoucherCat L ON H.NCat = L.NCat "
        HelpDataSet.Process = AgL.FillData(mQry, AgL.GCn)

        mQry = " SELECT H.Code, H.Description AS Item, IsNull(H.IsDeleted,0) AS IsDeleted, " & _
                " IsNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status, H.Div_Code " & _
                " FROM Item H "
        ...
        mQry = " SELECT H.Code AS Code, H.Description AS ItemGroup FROM ItemGroup H  "
        HelpDataSet.ItemGroup = AgL.FillData(mQry, AgL.GCn)

        mQry = " SELECT H.Code AS Code, H.Description, IsNull(H.IsDeleted,0) as IsDeleted, IsNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status FROM JobWorkerRateGroup H  "
    ...
    Private Sub Dgl1_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Dgl1.CellEnter
        Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
            Case Col1JobWorker
                Dgl1.AgRowFilter(Dgl1.Columns(Col1JobWorker).Index) = " IsDeleted = 0 " & _
