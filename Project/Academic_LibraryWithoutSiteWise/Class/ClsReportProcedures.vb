...
#Region "Queries Definition"
    ...
    Dim mHelpBookPurchOrderNoQry$ = " Select DISTINCT Convert(BIT,0) As [Select],P.DocID AS Code,P.V_Type + ' - '+ Convert(NVARCHAR(5),P.V_No) AS [ORDER No] " & _
                                ...
                                " And ISNULL(P.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') = '" & AgTemplate.ClsMain.EntryStatus.Active & "'"
    ...
    Dim mHelpScrapCategory$ = " Select DISTINCT Convert(BIT,0) As [Select], I.Code ,I.Description AS [Scrap Category] " & _
                                ...
                                " WHERE ISNULL(I.IsDeleted,0)=0 And ISNULL(I.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') = '" & AgTemplate.ClsMain.EntryStatus.Active & "'"

    Dim mHelpGodownQry$ = " Select DISTINCT Convert(BIT,0) As [Select], Code,Description AS Godown FROM Godown " & _
                               " WHERE ISNULL(IsDeleted,0)=0 And ISNULL(Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') = '" & AgTemplate.ClsMain.EntryStatus.Active & "'"
    ...
    Dim mHelpMemberTypeQry$ = " SELECT Convert(BIT,0) As [Select], Mt.Code, Mt.Description AS MemberType FROM Lib_MemberType Mt"
    ...
    Dim mHelpBookTypeQry$ = " SELECT Convert(BIT,0) As [Select],H.Code AS Code, H.Description AS BookType FROM Lib_BookType H "
...
#Region "Payment Report"
    Private Sub ProcPaymentReport(ByVal bReportType As String)
        Try
            ...
            mCondStr = mCondStr & ObjRFG.GetWhereCondition("H.Site", 0)
            mCondStr = mCondStr & ObjRFG.GetWhereCondition("H.SubCode", 0)
