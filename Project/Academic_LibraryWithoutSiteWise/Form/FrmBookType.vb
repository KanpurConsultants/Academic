...
#End Region
    ...
    Private Sub FrmBookType_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From Lib_BookType Where Description='" & TxtDescription.Text & "' And " & ClsMain.RetDivFilterStr & ""
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("Book Type Already Exists") : passed = False : Exit Sub

            mQry = "Select count(*) From Lib_BookType_Log Where Description='" & TxtDescription.Text & "' And " & ClsMain.RetDivFilterStr & " And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("Book Type Already Exists in Log File") : passed = False : Exit Sub
        Else
            mQry = "Select count(*) From Lib_BookType Where Description='" & TxtDescription.Text & "' And Code<>'" & mInternalCode & "' And " & ClsMain.RetDivFilterStr & ""
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("Book Type Already Exists") : passed = False : Exit Sub

            mQry = "Select count(*) From Lib_BookType_Log Where Description='" & TxtDescription.Text & "' And UID <>'" & mSearchCode & "' And " & ClsMain.RetDivFilterStr & " And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("Book Type Already Exists in Log File") : passed = False : Exit Sub
        End If
    End Sub
            ...
            If .Rows.Count > 0 Then
                ...
                If FrmType = ClsMain.EntryPointType.Main Then
                    mQry = " SELECT MT.Code AS MemberType ,MT.Description,V1.Issuance,isnull(V1.Days,0) AS Days,isnull(V1.FinePerDay,0) AS FinePerDay,V1.UID,V1.Sr   " & _
                            ...
                            " WHERE isnull(MT.IsDeleted,0) = 0 AND isnull(MT.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "')= '" & AgTemplate.ClsMain.EntryStatus.Active & "' " & _
                            " Order By V1.Sr "
                Else
    ...
    Private Sub ProcFillMemberType()
        ...
        Try
            ...
            mQry = " SELECT MT.Code AS MemberType  FROM Lib_MemberType MT " & _
                    ...
                    " AND isnull(MT.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') = '" & AgTemplate.ClsMain.EntryStatus.Active & "' " & _
