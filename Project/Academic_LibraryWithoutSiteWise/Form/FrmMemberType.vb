...
#End Region
    ...
    Private Sub FrmMemberType_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From Lib_MemberType Where Description='" & TxtDescription.Text & "' And " & ClsMain.RetDivFilterStr & " aND IsNull(IsDeleted,0)=0 "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("Member Type Already Exists") : passed = False : Exit Sub
            mQry = "Select count(*) From Lib_MemberType_Log Where Description='" & TxtDescription.Text & "' And " & ClsMain.RetDivFilterStr & " And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
        ...
        Else
            mQry = "Select count(*) From Lib_MemberType Where Description='" & TxtDescription.Text & "' And Code<>'" & mInternalCode & "' And " & ClsMain.RetDivFilterStr & "  aND IsNull(IsDeleted,0)=0 "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("Member Type Already Exists") : passed = False : Exit Sub

            mQry = "Select count(*) From Lib_MemberType_Log Where Description='" & TxtDescription.Text & "' And UID <>'" & mSearchCode & "' And " & ClsMain.RetDivFilterStr & " And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("Member Type Already Exists in Log File") : passed = False : Exit Sub
        End If
